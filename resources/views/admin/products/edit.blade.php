                ...
                <div class="p-6 bg-white border-b border-gray-200">
                    <form action="{{ route('admin.products.update', $product) }}" method="POST" enctype="multipart/form-data" class="space-y-6">
                        ...
                        <div>
                            ...
                            <input type="number" id="stock" name="stock" value="{{ old('stock', $product->stock) }}" step="1" min="0" class="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-indigo-300 focus:ring focus:ring-indigo-200 focus:ring-opacity-50 @error('stock') border-red-500 @enderror" required>
                            @error('stock')
                                <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                            @enderror
                        </div>
                        ...
                        <div>
                            ...
                            <select id="platform" name="platform" class="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-indigo-300 focus:ring focus:ring-indigo-200 focus:ring-opacity-50 @error('platform') border-red-500 @enderror" required>
                                ...
                                <option value="Xbox" {{ old('platform', $product->platform) == 'Xbox' ? 'selected' : '' }}>Xbox</option>
                        ...
                        <div>
                            ...
                            <select id="brand" name="brand" class="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-indigo-300 focus:ring focus:ring-indigo-200 focus:ring-opacity-50 @error('brand') border-red-500 @enderror" required>
                                ...
                                <option value="Microsoft" {{ old('brand', $product->brand) == 'Microsoft' ? 'selected' : '' }}>Microsoft</option>
                                <option value="Nintendo" {{ old('brand', $product->brand) == 'Nintendo' ? 'selected' : '' }}>Nintendo</option>
                                <option value="PC" {{ old('brand', $product->brand) == 'PC' ? 'selected' : '' }}>PC</option>
                            </select>
                            @error('brand')
                                <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                            @enderror
